...
const LandingQRCode: React.FC<LandingQRCodeProps> = ({
  data, color, backgroundColor = "#ffffff", size = 100,
  dotType = "rounded", cornerSquareType = "extra-rounded", cornerDotType = "dot", logo,
}) => {
  ...
  useEffect(() => {
    const options: any = {
      ...
      cornersSquareOptions: { color, type: cornerSquareType },
      cornersDotOptions: { color, type: cornerDotType },
      backgroundOptions: { color: backgroundColor },
      qrOptions: { errorCorrectionLevel: "H" },
    };
    if (logo) {
      options.image = logo;
      options.imageOptions = { hideBackgroundDots: true, imageSize: 0.35, margin: 2, crossOrigin: "anonymous" };
    }
    qrCode.current = new QRCodeStyling(options);
    if (ref.current) { ref.current.innerHTML = ""; qrCode.current.append(ref.current); }
