...
const HeroSection = () => {
  ...
  return (
    <section className="relative pt-40 pb-6 md:pt-36 md:pb-12 lg:pt-40 lg:pb-16 min-h-105 md:min-h-120">
          ...
          <AnimatedSection delay={300}>
            <div className="flex flex-col gap-2 px-4 sm:px-0 sm:flex-row sm:gap-2 justify-center mb-6 sm:mb-8 md:mb-10">
              <a href="#generator" className="w-full sm:w-auto">
                <button className="px-6 gap-2 group w-full sm:w-auto text-sm h-10 sm:h-10 inline-flex items-center justify-center rounded-md bg-primary text-primary-foreground hover:bg-primary/90 transition-colors">
                  Create Free QR Code
                  <ArrowRight className="h-4 w-4 group-hover:translate-x-0.5 transition-transform" />
                </button>
              </a>
              <a href={`${APP_URL}/signup`} className="w-full sm:w-auto">
                <button className="px-6 gap-2 w-full sm:w-auto text-sm h-10 sm:h-10 inline-flex items-center justify-center rounded-md border border-border hover:bg-muted transition-colors">
          ...
          <AnimatedSection delay={400}>
            {isMobile ? (
              <div className="relative">
                <div className="absolute left-0 top-0 bottom-0 w-8 bg-linear-to-r from-background to-transparent z-10" />
                <div className="absolute right-0 top-0 bottom-0 w-8 bg-linear-to-l from-background to-transparent z-10" />
                <Marquee speed={25} className="py-2">
                  {stats.map((stat, index) => (
                    <div key={index} className="flex items-center gap-2 bg-muted/50 rounded-full px-4 py-2 shrink-0">
              ...
              <div className="grid grid-cols-4 gap-4 max-w-xl mx-auto">
                {stats.map((stat, index) => (
                  <div key={index} className="text-center">
                    <div className="text-xl md:text-2xl font-semibold text-foreground mb-0.5">{stat.value}</div>
